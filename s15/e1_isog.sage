import functools, time
print = functools.partial(print, flush=True)
exec(preparse(open('j2_rank.sage').read().split("for d3t, D in")[0]))
D = 5*13*65*65
cub = Ru(D * c * (uu^2 + A - 2*beta) * (uu + 2*sb))
a3, a2, a1, a0 = [cub[i] for i in (3, 2, 1, 0)]
E1 = EllipticCurve(k, [0, a2, 0, a1*a3, a0*a3^2]).global_minimal_model(semi_global=True)
curves = [("E1", E1)] + [(f"E1/<T{i+1}>", phi.codomain()) for i, phi in enumerate(E1.isogenies_prime_degree(2))]
for nm, E in curves:
    t0 = time.time()
    try:
        lo, hi, g = E.simon_two_descent()
        print(f"{nm:10s} torsion {E.torsion_subgroup().invariants()}  Simon rank bounds [{lo},{hi}]  ({time.time()-t0:.0f}s)")
    except Exception as ex:
        print(nm, "failed:", ex)
