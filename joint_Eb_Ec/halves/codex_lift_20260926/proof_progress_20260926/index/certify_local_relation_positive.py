...
S=PolynomialRing(QQ,'c');c=S.gen();K=S.fraction_field();b=QQ(24)/625
...
assert q(QQ(93)/100)*q(QQ(47)/50)<0
assert b+QQ(47)/50<1
out={'real_root_interval':['93/100','47/50'],'real_all_nine_positive':True,'b':str(b),'factor_degrees':[(int(t.degree()),e) for t,e in fac], 'q_coefficients':[str(t) for t in q.list()], 'exact_gcds':{name:str(q.gcd(S(v))) for name,v in polys.items()},'places':[]}
for p,c0 in [(2,8),(3,6)]:
...
print(json.dumps(out,indent=2));open('local_relation_positive_certificate.json','w').write(json.dumps(out,indent=2))
