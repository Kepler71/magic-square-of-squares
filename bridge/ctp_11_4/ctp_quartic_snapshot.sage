...
class FisherCTP:
    ...
    def local_point(self, g1, gam, pl, tries=60000):
        ...
        if isinstance(pl, RealPlace):
            ...
            pts += [RF(0), RF(1), RF(-1), RF(10), RF(-10), RF(100), RF(-100),
                    RF(1) / 10, RF(-1) / 10, RF(1) / 1000, RF(-1) / 1000]
            if rts:
                pts += [r * (1 + RF(t) / 1000) for r in rts for t in (-7, -3, 3, 7)]
            for x0 in pts:
                for jj in range(60):
                    # знаменатель должен позволять попасть в узкий положительный интервал
                    dmax = 10 ^ 6 if abs(x0) < 10 ^ 6 else ZZ(10) ^ 6
                    xq = QQ(x0.nearby_rational(max_error=abs(x0) / 10 ^ 12 + RF(1) / dmax))
                    if jj:
                        xq = xq * (1 + QQ(random.randint(-1000, 1000)) / 10 ^ 6)
                    x = k(xq); z = k(1)
                    v = quartic_eval(g1, x, z)
                    if v != 0 and gamv(x, z) != 0 and pl.is_sq(v):
                        return x, z
