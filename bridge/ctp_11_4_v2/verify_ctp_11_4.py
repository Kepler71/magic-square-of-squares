...
def selftest_hilbert(trials=400, seed=20260912):
    ...
    def soluble(a, b, p):
        """Разрешимо ли a x^2 + b y^2 = z^2 над Q_p (нетривиально).

        ПРАВКА 12.09: прежняя версия перебирала пары (x,y) двойным циклом до M = p^5,
        то есть ~2.8e8 итераций на каждую тройку (a,b,p) при p = 7, и скрипт не завершался
        (зависание обнаружено при состязательной проверке (11,4)).
        Перебор был избыточен: коника нормируется. Деля на y^2 (или на x^2), получаем
            разрешимо  <=>  a — квадрат, или b — квадрат,
                            или существует u с a*u^2 + b ненулевым квадратом.
        Случаи x = 0 и y = 0 — это ровно первые два. Достаточно одного цикла по u = p^j * w,
        где w пробегает единицы по модулю p^k: квадратность в Q_p* определяется
        конечной точностью (k = 3 при нечётном p, k = 5 при p = 2 — с запасом).
        Математическое содержание теста не изменилось, изменилась только стоимость.
        """
        if p == "real":
            return not (a < 0 and b < 0)
        p = int(p)
        if is_local_square(a, p) or is_local_square(b, p):
            return True
        k = 5 if p == 2 else 3
        M = p ** k
