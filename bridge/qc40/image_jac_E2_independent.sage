#!/usr/bin/env sage
# image_jac_E2_independent.sage -- НЕЗАВИСИМАЯ перепроверка §6 из basis_E2.sage:
...
#
# Запуск: sage /home/kep/magicKube/bridge/qc40/image_jac_E2_independent.sage
...
def psi_of_mumford(P, p, e1F, e2F, e1K, e2K, K, phi_embed):
    """P -- элемент J(F_p) в split-представлении Sage. Возвращает (A, B) в E1(F_p) x E2(F_p)."""
    u = P[0]
    v = P[1]
    n = ZZ(P[2]) if len(P) > 2 else None
    du = u.degree()
    A = e1K(0)
    B = e2K(0)
...
def local_image(p):
    ...
    img = {}
    for z in pts:
        img[z] = psi_of_mumford(z, p, e1F, e2F, e1K, e2K, K, None)
    # ПРОВЕРКА СОГЛАШЕНИЯ: Psi -- гомоморфизм, Psi(0) = 0
    zero = JF(0)
    assert img[zero] == (e1F(0), e2F(0)), "Psi(0) != 0 при p=%s" % p
    L = list(pts)
    for _ in range(200):
...
for p in primes_old + primes_new:
    if (D1 * D2) % p == 0:
        continue
    nJ, NE, S, idx, e1F, e2F = local_image(p)
    ok = set()
    ...
    say("[1]      накопленное пересечение: %s" % sorted(survivors))
...
(OUT / 'image_jac_E2_independent.json').write_text(
    json.dumps(R, indent=2, ensure_ascii=False) + '\n')
(OUT / 'image_jac_E2_independent.log').write_text("\n".join(LOG) + "\n")
say("записано: image_jac_E2_independent.json / .log")
