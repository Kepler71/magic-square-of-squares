...
class CTP:
    ...
    def real_values_certified(self, emb, vals, L):
        ...
        for prec in [256, 1024, 4096, 16384, 65536]:
            ...
            Lv = {j: sum(ev(L[j].monomial_coefficient(self.u[i])) * us[i] for i in range(4)) for j in (0, 1, 2)}
            if all(not Lv[j].contains_zero() for j in (0, 1, 2)):
                return {j: (1 if Lv[j] > 0 else -1) for j in (0, 1, 2)}, prec
            ...
            for P in Ps:
                ...
                while True:
                    for _try in range(50):
                        ...
                        if all(Lv[j] != 0 for j in (0, 1, 2)): break
                    else:
                ...
                if self.cert is not None:
                    self.cert.append({'place': str(P), 'gens_of_place': [str(g) for g in P.gens()],
                                      ...
                                      'L_values': {str(j): str(Lv[j]) for j in (0, 1, 2)},
                                      'v_L': {str(j): int(Lv[j].valuation(P)) for j in (0, 1, 2)},
                                      'f1': str(f1), 'f2': str(f2),
