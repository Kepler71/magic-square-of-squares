...
def E1_roots(A, C, D):
    ...
    return k, rts
for name, A, C, D in [('s=1/5 (17,7,13)', 109, 229, 65), ('(23,7,17)', 169, 409, 34), ('(71,49,61)', 3061, 4381, 671)]:
    t0 = time.time()
    k, rts = E1_roots(A, C, D)
    Cv = Curve3(k, rts)
    print(f"{name}: k = {k}, roots {rts}, S = {[P.smallest_integer() for P in Cv.S]}, dim k(S,2) = {Cv.n}")
    Sel = Cv.selmer_full()
    print(f"   full 2-descent: dim Sel2 = {Sel.dimension()} -> rank <= {Sel.dimension() - 2}   ({time.time()-t0:.0f}s)")
    for i in range(3):
        s1, s2 = Cv.isogeny_descent(i)
        print(f"   2-isogeny, kernel <(e{i+1},0)>: dim Sel(alpha) = {s1}, dim Sel(alpha') = {s2} -> rank <= {s1 + s2 - 2}   ({time.time()-t0:.0f}s)")
